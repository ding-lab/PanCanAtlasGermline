...
manhattan <- function(dataframe, colors=c("gray10", "gray50"), ymax="max", limitchromosomes=1:23, suggestiveline=-log10(1e-5), genomewideline=-log10(5e-8), annotate=NULL, ...) {
    ...
    if (numchroms==1) {
        with(d, plot(pos, logp, ylim=c(0,20), ylab=expression(-log[10](italic(p))), xlab=paste("Chromosome",unique(d$CHR),"position"), ...))
    }	else {
        with(d, plot(pos, logp, ylim=c(0,20), ylab=expression(-log[10](italic(p))), xlab="Chromosome", xaxt="n", type="n", ...))
