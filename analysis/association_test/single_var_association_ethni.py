...
def main():
    ...
    USAGE: single_association.py [-h] <exac vcf> <output file> <ethnicity> # get frequency to variants
    ...
    foutput = args[1] + "." + ethnicity + ".tsv"
    fout = open(foutput, 'w')
    # check what are the nonTCGA tag we have for these

    header_string = "Var\tExAC_AN_" + ethnicity+ "\tExAC_AC_" + ethnicity+ "\tExAC_nonTCGA_AN_" + ethnicity+ "\tExAC_nonTCGA_AC_" + ethnicity+ "\tTCGA_AN_" + ethnicity+ "\tTCGA_AC_" + ethnicity+ "\tOR\tP\tgene_symbol\timpact\tHGVSp\n"   
    ...
    for line in exacF:
        ...
        infos = info.split(";")
        for item in infos:
            if item.startswith("AC_" + ethnicity + "="):
                ACstr = item.replace("AC_" + ethnicity + "=","")
                ExAC_AC = int(ACstr)

            elif item.startswith("AN_" + ethnicity + "="):
                ANstr = item.replace("AN_" + ethnicity + "=","")
                ExAC_AN = int(ANstr)

            elif item.startswith("ExAC_nonTCGA_AC_" + ethnicity + "="): #ExAC_nonTCGA_AN_NFE
                AC_nonTCGAstr = item.replace("ExAC_nonTCGA_AC_" + ethnicity + "=","")
                ExAC_nonTCGA_AC = int(AC_nonTCGAstr)

            elif item.startswith("ExAC_nonTCGA_AN_" + ethnicity + "="):
                AN_nonTCGAstr = item.replace("ExAC_nonTCGA_AN_" + ethnicity + "=","")
