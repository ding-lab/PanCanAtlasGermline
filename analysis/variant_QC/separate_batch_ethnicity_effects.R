...
library("relaimpo")
library("hier.part")
istat_clin_coverage_pathCount_test = istat_clin_coverage_pathCount[complete.cases(istat_clin_coverage_pathCount[,c("NVAR","Center","Analyte","type","consensus_call")]),c("NVAR","Center","Analyte","type","consensus_call")]
hier.part(istat_clin_coverage_pathCount_test$NVAR, istat_clin_coverage_pathCount_test[,-1], family = "gaussian", gof = "RMSPE")

istat_clin_coverage_pathCount_test = istat_clin_coverage_pathCount[complete.cases(istat_clin_coverage_pathCount[,c("pathVarCount","Center","Analyte","type","consensus_call")]),c("pathVarCount","Center","Analyte","type","consensus_call")]
hier.part(istat_clin_coverage_pathCount_test$pathVarCount, istat_clin_coverage_pathCount_test[,-1], family = "gaussian", gof = "RMSPE")

# check for overall exomic variants
fit = lm(data = istat_clin_coverage_pathCount, NVAR ~ Center + Analyte + type + consensus_call)
summary(fit)
...
write.table(results, file=fn, quote=F, sep="\t", col.names=T, row.names=F)

# check for pathogenic variants
fit = lm(data = istat_clin_coverage_pathCount, pathVarCount ~ Center + Analyte + type + consensus_call)
summary(fit)
results = data.frame(anova(fit))
colnames(results) = gsub("\\.","",colnames(results))
results = rbind(results, colSums(results))
...
# titv 
fit = lm(data = istat_clin_coverage_pathCount, TITV ~ Center + Analyte + type + consensus_call)
summary(fit)
results = data.frame(anova(fit))
colnames(results) = gsub("\\.","",colnames(results))
results = rbind(results, colSums(results))
...
for (eth in unique(istat_clin_coverage_pathCount$consensus_call)){
...
istat_clin_coverage_pathCount_eur = istat_clin_coverage_pathCount[istat_clin_coverage_pathCount$consensus_call=="eur",]
...
for (gene in test_genes){
  ...
  for (cancer in unique(pathVarP_eur_g$cancer)){
    ...
    istat_clin_coverage_pathCount_eur_overall$inCancer[istat_clin_coverage_pathCount_eur_overall$type == cancer] = 1
    test_set = istat_clin_coverage_pathCount_eur_overall[istat_clin_coverage_pathCount_eur_overall$inCancer==1 | 
                                                         !(istat_clin_coverage_pathCount_eur_overall$type %in% cancer_to_exclude),]
...
# ##### plotting by cancer type and the specific variable of interest #####
# 
# ### all variants in exome ###
# p = ggplot(data=istat_clin_coverage_pathCount[!is.na(istat_clin_coverage_pathCount$consensus_call) & istat_clin_coverage_pathCount$consensus_call != "mix",],aes(x = consensus_call, y=NVAR))
# p = p + facet_grid(.~type, drop = T)
# p = p + geom_violin(alpha=0.8, aes(fill = consensus_call))
...
# 
# ### pathogenic variants ###
# p = ggplot(data=istat_clin_coverage_pathCount[!is.na(istat_clin_coverage_pathCount$consensus_call) & istat_clin_coverage_pathCount$consensus_call != "mix",],aes(x = consensus_call, y=pathVarCount))
# p = p + facet_grid(.~type, drop = T)
# p = p + geom_violin(alpha=0.8, aes(fill = consensus_call))
# p = p + stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median,
#                      geom = "crossbar", width = 0.8)
