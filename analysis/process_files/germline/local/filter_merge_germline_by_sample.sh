...
sample="${line%\\n}"
	...
	# remove original VCF
	rm -f ${batch}/${sampleName}.prefilter.snp_indel.vcf.gz
done < "$1"

#merge
~/bin/bcftools-1.5/bcftools merge --output-type z --output ${batch}.merge.vcf.gz $(ls -1 ${batch}/*.AD.5.vcf.gz | perl -pe 's/\n/ /g')
#index
