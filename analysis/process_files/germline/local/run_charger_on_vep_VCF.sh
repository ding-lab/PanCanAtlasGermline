...
inputDir="/gscmnt/gc3020/dinglab/medseq/Germline/projects/PanCanAtlasGermline/analysis/process_files/germline/local/new_run/AnnotatedVCFs"
...
for i in {1..22} X Y;
do
	sample="$inputDir/anno.PCA.r1.TCGAbarcode.merge.exon.chr${i}.vcf"
	vcf="PCA.r1.TCGAbarcode.merge.exon.chr${i}.vcf"
	output="${results}charged.${vcf}.tsv"
	command="charger --include-vcf-details -f ${sample} -o ${output} -O -D -g ${mmGenes} -z ${mmVariants} -H ${hotspot} -l --mac-clinvar-tsv ${clinvar} --rare-threshold ${rareThreshold} > ${results}charger.${vcf}.out"
