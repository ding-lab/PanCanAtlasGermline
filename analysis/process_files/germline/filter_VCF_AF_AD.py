...
def main():
    ...
    outFstring = "ExAC_AF." + str(MAF_thres) + ".AD." + str(AD_thres) + ".vcf"
        ...
        if line.startswith("#"):
            ...
            for info in info_f:
                ...
                if info.startswith("ExAC_AF_Adj"):
                    ExAC_AF = info.replace("ExAC_AC_Adj=","")
                    if "," in ExAC_AF:
                        ExAC_AFs = ExAC_AF.split(",")
                        if ExAC_AFs[0] > MAF_thres: # need to assume it's the first allele
                            nonpass_MAF = True
                    else:
                        if ExAC_AF > MAF_thres:
                            nonpass_MAF = True
            if nonpass_MAF:
