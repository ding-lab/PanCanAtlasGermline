...
run_glm = function(data=NULL, covi="") {
  ...
  ytype="Q";yi="log2RSEM";xi="Freq";covi=covi;#covi=md[i,4];memo=md[i,5]
  # DEBUG
...
exp_score_g = exp_score[exp_score$Gene %in% pathVarP$HUGO_Symbol,]
...
for (gene in genes){
  ...
  var_exp_g = merge(exp_score_g_g,gene_sample_g,by="bcr_patient_barcode",all.x=T)
  ...
  for (cancer in cancers){
    ...
    if (gene_path_count > 2){
      # run GLM
      w = wilcox.test(var_exp_g_c$log2RSEM[var_exp_g_c$Freq==0],var_exp_g_c$log2RSEM[var_exp_g_c$Freq!=0])
      wP = w$p.value
      wWstat = w$statistic
...
colnames(tt) = c("cancer","gene","gene_path_count","wilcoxP","W_stat","y","y_type","Gene","degrees_freedom","deviance","residual_degrees_freedom","residual_deviance",
