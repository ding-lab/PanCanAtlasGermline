...
tn = "out/germline_somatic_driver_fisher_byCancer.tsv"
tt = read.table(sep="\t",header=T,file=tn, stringsAsFactors=FALSE)

germlineG = unique(tt$GermlineGene[tt$P<0.05])
somaticG = unique(tt$SomaticGene[tt$P<0.05])
cancer = unique(tt$Cancer[tt$P<0.05])
ttG = tt[tt$GermlineGene %in% germlineG & tt$SomaticGene %in% somaticG & tt$Cancer %in% cancer,]
...
p = p + labs(title="Germline-somatic Interaction: Single-cancer",x="Germline variant carrier",y = "Somatic mutation carrier")
p
fn = 'out/pan10389_germlineAssocWithSomaticHeatmapByCancer.pdf'
