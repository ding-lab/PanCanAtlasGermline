...
all_test_samples = intersect(unique(somatic$bcr_patient_barcode), clin$bcr_patient_barcode)

# run through function
out_table=character(0);
for (cancer in unique(pathVarP$cancer)){
  test_samples = intersect(all_test_samples, clin$bcr_patient_barcode[clin$type==cancer])
  cat("Running germline-somatic interaction in ",length(test_samples),"samples.\n")
  
  pathVarPcancer =pathVarP[pathVarP$cancer==cancer,]
  somatic_likelyfunctional_driver_cancer = somatic_likelyfunctional_driver[somatic_likelyfunctional_driver$cancer==cancer,]
  
  for (g_gene in germ_list){
    for (s_gene in somaticDriver299){
      germline_carriers = unique(pathVarPcancer$bcr_patient_barcode[pathVarPcancer$HUGO_Symbol==g_gene])
      somatic_carriers = unique(somatic_likelyfunctional_driver_cancer$bcr_patient_barcode[somatic_likelyfunctional_driver_cancer$Hugo_Symbol==s_gene])
      if(length(germline_carriers) > 4 & length(somatic_carriers) > 9){
        ...
        out_row = c(cancer,g_gene, s_gene, count00, count10, count01, count11, OR, p)
...
colnames(out_table) = c("Cancer","GermlineGene", "SomaticGene", "germline0somatic0", "germline1somatic0", "germline0somatic1", "germline1somatic1","OR", "P")
...
fn = paste("out/germline_somatic_driver_fisher_byCancer.tsv")
